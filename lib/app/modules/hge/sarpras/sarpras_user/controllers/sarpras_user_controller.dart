...
class SarprasUserController extends GetxController {
  ...
  getSarpras(hal) async {
    await provider.getSarprasUser("$hal", nik.value).then((value) {
      if (value != null) {
        print("sarpras $value");
  ...
  void onLoading() async {
    if (page < lastPage) {
      ...
      page.value++;
      await provider.getSarprasUser("$page", nik.value).then((value) {
        if (value != null) {
          print("sarpras $value");
          var sarpras = value.sarpras;
          if (sarpras != null) {
