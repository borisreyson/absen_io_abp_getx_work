...
class MonitoringOBView extends GetView<MonitoringOBController> {
  ...
  Widget listOB() {
    return SmartRefresher(
      ...
      child: ListView(
        children:
            controller.dataOB.map((element) => contentOB(element)).toList(),
      ),
    );
  }

  Widget contentOB(DataOB ob) {
    var pd = controller.f.value.format(double.parse("${ob.planDaily}"));
    var ad = controller.f.value.format(double.parse("${ob.actualDaily}"));
    var pm = controller.f.value.format(double.parse("${ob.mtdPlan}"));
    var am = controller.f.value.format(double.parse("${ob.mtdActual}"));
